...
class DemoWriteGuard(BaseHTTPMiddleware):
    ...
    def __init__(self, app, capacity: int | None = None, window: float | None = None) -> None:
        super().__init__(app)
        # Read the module defaults at mount time so tests can monkeypatch
        # CAPACITY/WINDOW_SECONDS before create_app() (MC 707.2 G3).
        self._capacity = float(capacity if capacity is not None else CAPACITY)
        self._rate = self._capacity / float(window if window is not None else WINDOW_SECONDS)
